...
import { ExecuteStatementCommand, BatchExecuteStatementCommand, DecimalReturnType } from "@aws-sdk/client-rds-data";
...
describe("formatResults", () => {
  ...
  it("generatedFields: for insert query", () => {
    const obj = dbClient.formatResults({ $metadata: {}, generatedFields: [{ longValue: 5 }] })
  ...
  it("updateResults: for batch update query", () => {
    ...
    const obj = dbClient.formatResults({
      $metadata: {}, records, columnMetadata
    })
...
describe('buildCommand', () => {
  it('DB configs are copied', () => {
    ...
    const resourceArn = 'resource_arn', secretArn = 'resource_arn', database = 'test_db'
  ...
  it('DB configs can be overwritten', () => {
    ...
    const SQL = 'select * from test'
    const resourceArn = 'resource_arn', secretArn = 'resource_arn', database = 'test_db'
    const command = dbClient.buildCommand(SQL, { resourceArn, secretArn, database })
    const input = (command as ExecuteStatementCommand).input
    expect(input.sql).toEqual(SQL);
    expect(input.continueAfterTimeout).toBe(false);
    expect(input.resourceArn).toEqual(resourceArn);
  ...
  it('Params are processed for single query', () => {
    ...
    const command = dbClient.buildCommand(SQL, { params: { id: 101, pkg: 'package1' } })
    const input = (command as ExecuteStatementCommand).input
    console.log(input.parameters)
    expect(input.sql).toEqual(SQL);
    expect(input.continueAfterTimeout).toBe(true);
    expect(input.parameters).toEqual(expect.arrayContaining([
  ...
  it('Params are processed for batch query', () => {
    const dbClient = new DBClient(getConfig());
    const SQL = 'UPDATE test SET package_name = :pkg WHERE id = :id'
    const command = dbClient.buildCommand(SQL, { params: [{ id: 101, pkg: 'package1' }, { id: 102, pkg: 'package2' }] })
    ...
    expect(input.parameterSets).toEqual([
      [{ "name": "id", "value": { "longValue": 101 } }, { "name": "pkg", "value": { "stringValue": "package1" } }],
      [{ "name": "id", "value": { "longValue": 102 } }, { "name": "pkg", "value": { "stringValue": "package2" } }]
    ])
  })
  it('Valid params are passed', () => {
    const dbClient = new DBClient(getConfig());
    const SQL = 'DELETE FROM test where repo = :repo'
    const includeResultMetadata = true, resultSetOptions = { decimalReturnType: DecimalReturnType.STRING }, continueAfterTimeout = false
    const params = { id: 101, pkg: 'package1' }
    const command = dbClient.buildCommand(SQL, { params, includeResultMetadata, resultSetOptions, continueAfterTimeout })
    const keys = Object.keys(command.input)
    expect(keys).toEqual(expect.arrayContaining(['sql', 'resourceArn', 'secretArn', 'database', 'includeResultMetadata', 'resultSetOptions', 'parameters']))
    const input = (command as ExecuteStatementCommand).input
    expect(input.sql).toEqual(SQL);
  ...
  it('Unnecessary params are omitted', () => {
    const dbClient = new DBClient(getConfig());
    const SQL = 'DELETE FROM test where repo = :repo'
    const includeResultMetadata = true, resultSetOptions = { decimalReturnType: DecimalReturnType.STRING }, continueAfterTimeout = true
    const params = [{ id: 101, pkg: 'package1' }, { id: 102, pkg: 'package2' }]
    const command = dbClient.buildCommand(SQL, { params, includeResultMetadata, resultSetOptions, continueAfterTimeout })
    const keys = Object.keys(command.input)
    expect(keys).toEqual(expect.arrayContaining(['sql', 'resourceArn', 'secretArn', 'database', 'parameterSets']))
    expect(keys).toEqual(expect.not.arrayContaining(['includeResultMetadata', 'resultSetOptions', 'continueAfterTimeout', 'parameters']))
  })
});
...
describe('isDMLQuery', () => {
  ...
  it('for insert', () => {
    const bDML = dbClient.isDMLQuery(' Insert into test (repo,package_name) values(:repo,:package_name) ');
    expect(bDML).toBe(true)
  })
  it('for update', () => {
    const bDML = dbClient.isDMLQuery(' UPDATE test SET package_name = :pkg WHERE repo = :repo ');
    expect(bDML).toBe(true)
  })
  it('for delete', () => {
